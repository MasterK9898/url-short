  ...
  return (
    <div>
      ...
      <div className={classNames("membership")}>
        <div className={classNames("membership-title")}>Choose your plan</div>
        <div className={classNames("membership-prompt")}>
          "Unlock limitless possibilities and superior features by upgrading to
          our premium plan today!"
        </div>
        <div className={classNames("plans")}>
          {membershipDetail.map((detail) => (
            <PlanCard key={detail.id} planDetails={detail} />
          ))}
        </div>
        {/* <div className={classNames("day-pass")}>
          Or find our supplementary package{" "}
          <span
            className={classNames("purchase-day-pass")}
            onClick={() => {
              setSuppModal(true);
            }}
          >
      ...
      >
        ...
        <Modal.Body className={classNames("supp-description")}>
          <div
            className={classNames("one-time-prompt")}
            ...
            {demoPlans.map((plan, index) => (
              <div
                ...
                style={{
                  ...
                  height: "80%",
                  border:
                    selectedPack && selectedPack.quota === plan.quota
                      ? "2px solid #282c34"
                      : "1px solid #e0e0e0",
                  borderRadius: "8px",
                  padding: "15px",
              ...
              >
                ...
                >
                  <strong>{plan.description} Pack</strong>
                </div>
                <div style={{ fontSize: "14px", marginBottom: "8px" }}>
                  {plan.quota} Quotas
                </div>
                <div style={{ fontSize: "16px", fontWeight: "600" }}>
                  ${plan.price} <span style={{ fontSize: "10px" }}>+tax</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <Modal.Footer style={{ display: "flex", alignItems: "center" }}>
          <div>
            {selectedPack
              ? `Selected: ${selectedPack.quota} quota (${selectedPack.description})`
              : "No pack selected..."}
          </div>
          <Button
